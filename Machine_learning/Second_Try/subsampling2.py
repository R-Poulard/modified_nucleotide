...
df=pd.read_csv("~/M1/get_nodes/node2.csv")
...
one_hot = pd.get_dummies(df['at_1'],prefix="at_1")
df = df.drop('at_1',axis = 1)
df = df.join(one_hot)
one_hot=pd.get_dummies(df['at_-1'],prefix="at_-1")
df=df.drop('at_-1',axis=1)
df=df.join(one_hot)
print(df.head())
mat=df.to_numpy()
X=mat[:,1:]
...
def evaluate(pred_list,test):
    ...
    data.to_csv("~/M1/score_testing_subsampling_2.csv",header=True,index=False)
    return None
def research_best(X,y):
    ...
    with open(os.path.join(".","grid_cv2.pickle"), 'wb') as handle:
    ...
    with open(os.path.join(".","grid_KN2.pickle"), 'wb') as handle:
    ...
    with open(os.path.join(".","grid_RF2.pickle"), 'wb') as handle:
